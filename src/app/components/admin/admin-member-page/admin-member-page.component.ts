...
export class AdminMemberPageComponent {
  ...
  onSubmit() {
    ...
    if (values.selection === 'number') {
      ...
      if (!values.keyWord) {
        ...
        this.adoptionService.onQueryUserByUserId(Number(values.keyWord)).subscribe((res) => {
          if ('statusCode' in res) {
            console.log('查無資料')
          }
          if ('user' in res) {
    ...
    if (values.selection === 'email') {
      if (!values.keyWord) {
        ...
        this.adoptionService.onQueryUsersByGoogleAccount(values.keyWord).subscribe((res) => {
          if ('statusCode' in res) {
            console.log('查無資料')
          }
          if ('users' in res) {
  ...
  putStatusOn() {
    ...
    if (userId) {
      const index = this.displayUsers.findIndex(user => user.userId === Number(userId));
      if (this.displayUsers[index].status === 'Y') {
        return
      } else {
        this.adoptionService.onPutUserStatusOn(Number(userId)).subscribe((res) => {
          if (res.statusCode) {
            console.log('成功');
            this.onReset()
          } else {
  ...
  putStatusOff() {
    ...
    if (userId) {
      ...
      if (this.displayUsers[index].status === 'N') {
        ...
        console.log('off')
        this.adoptionService.onPutUserStatusOff(Number(userId)).subscribe((res) => {
          console.log(res)
          if (res.statusCode) {
            console.log('成功');
            this.onReset()
